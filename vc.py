...
def run_noxim(load, routing, mesh_size, traffic , vc):
    command = f"{bin_path} -pir {load / 8} poisson -vc {vc}  -dimx {mesh_size} -dimy {mesh_size} -routing {routing} -traffic {traffic}"
...
def main():
    ...
    for x in range(len(lat)):
        ax1.plot(loads, lat[x], label = f'#vc = {sizes[x]}')
        ax2.plot(loads, thr[x], label = f'#vc = {sizes[x]}')
