...
def run_noxim(load, routing, mesh_size, traffic , packet):
    # Construct the Noxim command with the specified load, algorithm, network, and traffic
    command = f"{bin_path} -pir {load / 8} poisson -size {packet} {packet} -dimx {mesh_size} -dimy {mesh_size} -routing {routing} -traffic {traffic}"
...
def main():
    ...
    for x in range(len(lat)):
        ax1.plot(loads, lat[x], label = f'packet size = {sizes[x]}')
        ax2.plot(loads, thr[x], label = f'packet size = {sizes[x]}')
